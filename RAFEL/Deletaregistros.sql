CREATE OR REPLACE PROCEDURE REMOVERG (BPARAMETRO1 IN BOOLEAN DEFAULT TRUE)
   IS
   
CURSOR C1 IS SELECT A.codigo, A.nome FROM PPESSOA A
   WHERE not( exists (SELECT 0 FROM AUSOPROV B WHERE B.codpessoa = A.CODIGO)
...
BEGIN
   IF BPARAMETRO1 THEN
      DBMS_OUTPUT.put_line('REMOÇÃO DE REGISTROS SEM VINCULO');
      DBMS_OUTPUT.put_line('EXECUÇÃO');
   END IF;
   FOR RC1 IN C1 LOOP
      IF BPARAMETRO1 THEN
         DBMS_OUTPUT.put_line(RC1.codigo || ' - ' || RC1.nome);
      END IF;
      DELETE PPESSOA WHERE CURRENT OF C1;
   END LOOP;
--   COMMIT;
END; -- Procedure
/
