CREATE OR REPLACE PROCEDURE REMOVEESPACO2
   IS
CURSOR C1 IS
   SELECT NOME, CODIGO
      FROM ppessoa a
...
BEGIN
   DBMS_OUTPUT.put_line('REMOÇÃO DOS ESPAÇOS DO FINAL DO NOME');
   WHILE NOT BFLAG LOOP
      ...
      FOR RC1 IN C1 LOOP
         ...
         IF NNOME > 0 THEN
            ...
            ELSE
               UPDATE VUSUARIOCURRICULO SET NOME = trim(trailing ' ' from NOME) WHERE codpessoa = RC1.CODIGO;
         ...
         IF NNOME > 0 THEN
            ...
            ELSE
               UPDATE PFUNC SET NOME = trim(trailing ' ' from NOME) WHERE codpessoa = RC1.CODIGO;
         ...
         IF NNOME > 0 THEN
            ...
            ELSE
               UPDATE EALUNOS SET NOME = trim(trailing ' ' from NOME) WHERE codpessoa = RC1.CODIGO;
         ...
         IF NNOME > 0 THEN
            ...
            ELSE
               UPDATE EPROFES SET NOME = trim(trailing ' ' from NOME) WHERE codpessoa = RC1.CODIGO;
               SSTRING_SAIDA := SSTRING_SAIDA || 'EPROFES - ';
            END IF;
         END IF;
         UPDATE PPESSOA SET NOME = trim(trailing ' ' from NOME) WHERE CURRENT OF C1;
